...
namespace modulo {
    ...
    struct __mint {
        ...
        __mint(long long x = 0) {
          if (x < 0) x = x % modSeed + modSeed;
          if (x >= modSeed) x %= modSeed;
          val = intType(x);
        }

        __mint(unsigned long long x) {
          if (x >= modSeed) x %= modSeed;
          val = intType(x);
        }

        __mint(int x) : __mint((long long) (x)) {}

        __mint(unsigned x) : __mint((unsigned long long) (x)) {}
        ...
        static unsigned fast_mod(unsigned long long x, unsigned m = modSeed) {
...
//    private: //! Use only when needed to avoid silly bugs
    ...
    struct metrics {
        ...
        long long prepared_maximum = -1;

        explicit metrics(long long maximum) {
          static long long prepare_calls = 0;
          if (prepare_calls++ == 0) {
            // Make sure modSeed is prime, which is necessary for the inverse algorithm below.
            for (long long p = 2; p * p <= modSeed; p += p % 2 + 1)
              assert(modSeed % p != 0);
            inv = {0, 1};
            fac = invFac = {1, 1};
            prepared_maximum = 1;
          ...
          if (maximum > prepared_maximum) {
            ...
            for (long long i = prepared_maximum + 1; i <= maximum; i++) {
              inv[i] = inv[modSeed % i] * (modSeed - modSeed / i);
              fac[i] = i * fac[i - 1];
              invFac[i] = inv[i] * invFac[i - 1];
            }

            prepared_maximum = (long long) (maximum);
          }
        }
        ...
        mint inv_nCr(long long n, long long r) {
          assert(0 <= r && r <= n);
        ...
        mint inv_nPr(long long n, long long r) {
          assert(0 <= r && r <= n);
